...
/**
 * @brief Creates a new Jerry object with the given origin, ID, and happiness level.
 *        Allocates memory dynamically for the Jerry and its ID.
 *
 * @param origin Pointer to the Origin object representing Jerry's origin.
 * @param id String representing Jerry's unique ID.
 * @param happinessLevel Integer representing Jerry's happiness level.
 * @return Pointer to the newly created Jerry object on success, or NULL on memory allocation failure.
 */

Jerry* createJerry(Origin* origin, char* id, int happinessLevel) {
...
status destroyJerry(Jerry* p_jerry) {
    ...
    if (p_jerry->origin != NULL) {
        ...
        if (p_jerry->origin->planet != NULL) {
            if (p_jerry->origin->planet->name != NULL) {
                free(p_jerry->origin->planet->name);
                p_jerry->origin->planet->name = NULL; //set to null
            }
